...
- **Automated login via IBeam** — IB Client Portal Gateway only accepts authentication through its own web UI; programmatic credential POSTs return 401, and exposing the gateway's UI through a Cloudflare Quick Tunnel doesn't carry the session cookies cleanly back to internal API queries (both discovered during Batch 13). We use [IBeam](https://github.com/Voyz/ibeam) (`voyz/ibeam:0.5.11`) — it runs the gateway inside its own container and drives the browser-based login at `localhost:5000` via a headless browser. The api treats the IBeam container exactly like the bare gateway: same `ib-gateway:5000` hostname, same Client Portal REST endpoints.
- **Tried and abandoned** (kept here as institutional memory so the next agent doesn't redo): (1) programmatic POST to `/v1/api/iserver/auth/ssodh/init` → 401; (2) api-side path proxy `/ib-portal/*` → gateway's absolute paths bypassed the prefix; (3) second dedicated Quick Tunnel to the gateway → "login succeeded" in the browser but internal queries still got 401; (4) IBKR's own OAuth 1.0a Extended — works, but requires IBKR-side activation that retail accounts have to request explicitly via email and may wait days-to-weeks for. OAuth implementation is parked on the `oauth-dev` branch for when approval lands.
...
- **2FA**: when IBeam initiates login, IBKR fires a push to your IB Key phone app — approve with biometrics, IBeam captures the post-2FA session, and the gateway is authenticated. Daily nightly forced logout (~11:45 PM ET) triggers IBeam to re-login automatically.
- **No Redis token storage in the BE for IB session** — IBeam/Gateway hold the session internally; the api just polls `/v1/api/iserver/auth/status` and surfaces the result via `/api/auth/status` to the FE.
