...
### Batch ARCH-9: Scheduler primitive (Phase 3) — defineCron + 4-cron reference set — COMPLETE
First Phase-3 slice. `kernel/scheduler.ts` exports the one cron primitive `defineCron({ name, intervalMs, firstRunDelayMs?, gates?, run }) → { start, stop }` — **single-flight by construction** (next tick scheduled in the `finally` after `run` settles, so a slow body never overlaps; the recursive `setTimeout` *is* the lock — **no `lock` knob**, we run one api instance and won't imply a cross-instance guarantee). Uncaught throw → `notifyError(`${name}.tick`)`, loop survives. **Gates** (`cron/gates.ts`: `ibAuthGate`/`marketRegularGate`/`marketRegularOrAfterHoursGate`) are an AND-composed "skip this tick" seam kept out of the kernel so it stays vendor-agnostic — **distinct from the job queue's per-action preconditions** (`services/jobs/gates.ts` + `requiresRthOpen`, which defer *jobs*, not loop ticks). Phased ahead of the bulk rollout: a 4-cron reference set migrated to prove the abstraction — `lockCleanup` (no gate; `setInterval`→single-flight), `keepalive` (1 file→2 handles, each own error policy), `riskFlagsCron` (`gates:[ibAuthGate]`, drops inline `status()` — the template for the other 6 IB crons), `catalystReversalProducer` (2 loops→2 handles; job-layer gates untouched). `index.ts` unchanged (`startXxx()` kept). No behavior change (cadences/first-delays/gating preserved); audit-only delta: catalyst loop-error keys gain `.tick`. Typecheck clean (incl. scripts); 235/235 (+14). Commits `a059742`/`5e8781b`/`a98816f`. **Next: ARCH-10** = bulk rollout of the remaining ~20 crons onto `defineCron` (7 IB crons → `gates:[ibAuthGate]`; bandEngine/ibPricePoller add a market gate; retention/cleanup are no-gate).

### Batch ARCH-8: Ports & adapters (Phase 2) — ib gateway adapter (HTTP half) — COMPLETE
Last (and largest) Phase-2 slice; **Phase 2 now complete** (bar the deferred `llm`). `services/ibGateway.ts` inverted into `adapters/ib/` in two commits. **8a (`9d3c2f0`)** grew `HttpAdapter.instrumented()` for the things finnhub didn't exercise, each an opt-in flag so the finnhub/polygon/yahoo paths stay byte-identical: a caller-bumpable retry counter (`request` receives a `retry()` for ibSnapshot's subscribe-then-poll loop), `detail: "after N retries"`, `skipNotify` (the debug passthrough records a metric but never pings Discord), `rawData` (return the body even on a non-2xx so the passthrough relays IB's actual error) — folding the old `instrumented`/`instrumentedWithRetry` pair into the one base method; +6 base tests (`HttpAdapter.test.ts`). **8b (`96509a8`)** stood up `port.ts` (`IbGatewayPort` — 16 vendor-shaped methods + the `RawIbTransaction`/`RawIbTrade`/`IbRawResponse` wire types) + `ibGatewayAdapter.ts`, now the **SOLE `env.ibGatewayUrl` reader** / sole path to the gateway (owns the client + self-signed `httpsAgent` via a new optional `HttpAdapter` `clientConfig` ctor arg + the 100ms rate limit + all ~18 calls + the snapshot retry + the raw passthrough; auth/session calls stay un-instrumented). Pure entry-date deduction split to `adapters/ib/entryDeduction.ts`. **20** callers rewired to the `ibGateway` singleton (the brief's "15" undercounted — `services/*` import via `./ibGateway.js`), method names dropping the redundant `ib` prefix; incl. `owner.ts`'s inline `/iserver/accounts` axios call (the one other `env.ibGatewayUrl` reader) folded into `ibGateway.accounts()` — **that flips the grep gate green**. `services/ibGateway.ts` deleted; `ibContainer`/`ibMappers`/`ibPassthroughAllowlist` stay put. No control-flow change; two surfaced audit-only deltas (metric `endpoint` now vendor-namespaced `ib:<path>`; ibTransactions/ibTrades non-2xx warn drops the now-nulled body — still reaches Discord) + one narrowing (no status-0 metric/notify on a thrown request; redundant with /healthz). Typecheck clean; 221/221 (+6); grep gate clean; no migration. Commits `9d3c2f0`/`96509a8`/`d0a6fb5`. **Next: Phase 3** (`schedule()` / `defineCron` — Batch ARCH item 1) → Phase 4 (pure core into `domain/`).

### Batch ARCH-7: Ports & adapters (Phase 2) — finnhub adapter — COMPLETE
Fourth Phase-2 slice, and the one that makes `HttpAdapter` earn its keep. `services/finnhub.ts` inverted into `adapters/finnhub/`: `port.ts` (`FinnhubPort` + the five vendor types — `FinnhubQuote`/`FinnhubProfile2`/`FinnhubMetrics`/`FinnhubSymbolRow`/`FinnhubEarningsRow`) + `finnhubAdapter.ts`, now the **SOLE importer of `env.finnhubApiKey`** and sole path to `finnhub.io`. `finnhubQueue.ts` **moved in** as the adapter's own quirk (the global token-bucket + per-(category,key) min-interval ceiling polygon/yahoo lack; auth token rides as a `?token=` query param). **The timing → `external_api_metrics` → `notifyApiFailure` instrumentation the old file hand-rolled now lives in `HttpAdapter.instrumented()`** — provider/endpoint/notify-key derive from the `vendor` ctor field (`<vendor>` / `<vendor>:<category>` / `<vendor>_api.<category>`), so it's shared, not finnhub-specific; polygon/yahoo keep calling raw `get()` (their callers own their notify policy) and never touch `instrumented()`. The queue's defensive 429-retry stays put (a network-error retry). All 9 callers rewired from free functions to the `finnhub` singleton; the two externally-imported types repointed to the port; `services/finnhub.ts` deleted. No behavior change — same endpoints, params, queueing, mapping. Grep gate clean (`finnhub.io` / `env.finnhubApiKey` / `finnhubQueue` only under `adapters/finnhub/`); 215/215 (+7 adapter tests: 5 parsing + 2 instrumentation). Commit 74ecc86. Last Phase-2 vendor: `ib` (own multi-slice sub-batch, the second `instrumented()` consumer); `llm` deferred (roadmap Track 4).

### Batch ARCH-6: Ports & adapters (Phase 2) — discord adapter — COMPLETE
Third Phase-2 slice. Gated Discord delivery behind a single adapter: `notify.ts` inverted onto a new `adapters/discord/` — `port.ts` (`Notifier`: `post(channel, payload)` + `has(channel)` over nine logical `DiscordChannel` names) + `discordAdapter.ts`, now the **SOLE importer of the nine `env.discord*WebhookUrl` fields**. The adapter owns channel→URL resolution (incl. the critical→routine fallback), the 5s-timeout `axios.post`, the never-throw swallow, and no-op-when-unconfigured. Discord is a multi-full-URL delivery sink (one webhook per channel), not a single-base REST API, so the adapter does **not** extend `HttpAdapter` (the "implement their own version" case) — delivery is an injectable constructor seam for tests. `notify.ts` keeps all **policy**: the per-key cooldown state, severity→color, and all 14 alert formatters; it now calls `discord.post(channel, …)` instead of reading webhook URLs + posting. `has()` preserves the exact early-bail-before-cooldown semantics of the old `webhookFor()`/`!url` guard. No external caller changed (all still import the same public `notify*` fns). No behavior change — same channels, payloads, fallback, no-op semantics; grep gate clean (`env.discord*` only under `adapters/discord/`); 208/208 (+4 adapter tests). Commits: f31b8fe (adapter + notify.ts) · `docs(arch):` progress note. Next Phase-2 slice: `finnhub` (`finnhub.ts` + `finnhubQueue.ts` → `FinnhubPort`, absorbs the hand-rolled instrumentation into `HttpAdapter`), then `ib` (own sub-batch, last).
