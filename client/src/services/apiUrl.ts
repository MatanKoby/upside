...
// URL is written by the cloudflared tunnel watcher in the BE (see
...
// Subscribes to Supabase Realtime on app_config.api_url and refreshes the
// cache when the row changes. Call once at app mount; the returned function
...
export function subscribeToApiUrl(): () => void {
  const apply = (next: unknown): void => {
    ...
    console.log(`[apiUrl] api_url update: ${inMemoryUrl ?? '(none)'} -> ${next}`);
    inMemoryUrl = next;
