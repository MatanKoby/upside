...
- **Progress:** ARCH-1…7 shipped — **Phase 1 (TableModules) complete** + **Phase 2 (ports & adapters) underway**: ARCH-5 stood up the `HttpAdapter` base + `adapters/` root + the `polygon`/`yahoo` reference adapters + the `db/ → adapters/supabase/` relocation; ARCH-6 added the `discord` adapter (`notify.ts` → `Notifier`); ARCH-7 added the `finnhub` adapter (`FinnhubPort` + the `finnhubQueue` quirk; the hand-rolled metric/notify instrumentation landed on `HttpAdapter.instrumented()`) — see `BUILD_QUEUE_DONE.md` + `docs/arch/target-architecture.md` → Phase 2. **Next claimable: ARCH-8 = the `ib` gateway adapter (HTTP half only)** — a two-commit sub-batch: **8a** grows `HttpAdapter.instrumented()` for IB's internal-retry counter + `debug-passthrough:` notify-skip (+ base tests); **8b** stands up `IbGatewayPort` + `IbGatewayAdapter extends HttpAdapter` (owns the client + all ~18 calls + the raw passthrough), rewires the 15 callers, deletes `services/ibGateway.ts`, flips the grep gate (`env.ibGatewayUrl` only under `adapters/ib/`). **`ibContainer.ts` stays a plain service** (decided 2026-06-11 — no env secret / nothing to grep-gate; revisit only if Batch 13.3's reconciler needs the Docker test seam); `ibMappers`/`ibPassthroughAllowlist` stay put (pure). See `docs/arch/target-architecture.md` → Phase 2 rollout slice 4. `llm` deferred (roadmap Track 4).
