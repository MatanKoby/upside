...
- **Progress:** ARCH-1…7 shipped — **Phase 1 (TableModules) complete** + **Phase 2 (ports & adapters) underway**: ARCH-5 stood up the `HttpAdapter` base + `adapters/` root + the `polygon`/`yahoo` reference adapters + the `db/ → adapters/supabase/` relocation; ARCH-6 added the `discord` adapter (`notify.ts` → `Notifier`); ARCH-7 added the `finnhub` adapter (`FinnhubPort` + the `finnhubQueue` quirk; the hand-rolled metric/notify instrumentation landed on `HttpAdapter.instrumented()`) — see `BUILD_QUEUE_DONE.md` + `docs/arch/target-architecture.md` → Phase 2. **Next claimable: ARCH-8 = the `ib` adapter** — its own multi-slice sub-batch (the monster: 20+ files, app-owned gateway lifecycle via `ibContainer`, 503-off-hours; the second `instrumented()` consumer), deliberately last. `llm` deferred (roadmap Track 4).
