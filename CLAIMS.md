...
- **Pending before Completed:** (1) ✅ migration applied; (2) **re-run `./bin/upside rebuild`** — the float + prompt fixes are newer than the user's first rebuild; (3) get the new FE bundle live (hard-refresh / confirm Vercel builds from `dev`); (4) re-Analyze BBAI on the new FE — confirm the 3-leg playbook renders with single clean pill, and the read is less sell-biased. Then move to Completed.
