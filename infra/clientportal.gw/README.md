...
```bash
docker run --rm -p 5000:5000 --name cpg upside/clientportal.gw
```

- `--rm` auto-removes the container when stopped (no state lingers).
- `-p 5000:5000` exposes the gateway on `https://localhost:5000` on the host.
...
2. Accept the self-signed cert warning (this is a localhost-only cert IB
   generates inside the container; nothing leaves your machine).
