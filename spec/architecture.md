...
- **Cached-first portfolio display**: the FE shows portfolio positions from Supabase regardless of whether IB is currently connected. When connected, `ibPricePoller` writes fresh data. When IB is disconnected, the Finnhub fallback poller keeps the canonical quote (`quotes.canonical_price`) reasonably fresh (see "Multi-source price polling"). The status indicator's color tells the user how fresh data is. **No full-screen "session expired" takeover.**
...
Both pollers recompute zone state on every write (see `signals/zone.md`). The FE renders the canonical price agnostic to source. **Price + P&L are no longer written to `positions` (Batch X5)** — the pollers write the price into `quotes` and only the price-source metadata onto `positions`.

**Watchlist coverage (post-watchlist-pivot, Batch A1):** the polling loop's symbol set is `held_conids ∪ active_watchlist_conids` — the union of held positions and tickers in `watchlist_lists WHERE active=true`. Same cadence, same IB-primary/Finnhub-fallback semantics. The canonical price for every tracked conid lands in `quotes` (Batch X5: that's the *only* price home — `positions` no longer mirrors it). Hidden watchlists are not polled. Discord channels extend correspondingly: `#upside-zone-profit` (held), `#upside-dip-buys` (watchlist markers + entry zones — see `signals/markers.md` and `signals/entry-zones.md`).
...
**Implemented (Batch X5):** the canonical price lives in **one** table — **`quotes`**, keyed by `conid` (see `schema.md`). `positions` holds holding facts only (conid, shares, avg_cost, realized_pnl, vwap, zone-state, entry provenance) and carries **no** price or price-derived column. Every price surface — position cards, watchlist rows, TickerDetail, chart, sparkline — reads `quotes.canonical_price` by conid. **Market value + unrealized P&L are recomputed** from `quotes.canonical_price × shares` (the FE in `usePositions`/`useTickerDetail`; the server in `routes/portfolio` `/summary` and `signalEngine`), never stored on `positions`. The table stores **both** IB and Finnhub prices side-by-side (each with its own timestamp) so divergence is observable and fallback is made on real provenance — not by silently overwriting one source with the other; the pollers cover every tracked conid (held ∪ active-watchlist), writing the quote + only price-source metadata (`price_source`, `last_price_update_at`) onto `positions`.
