...
7. **Continuous evaluation (mechanical leg tracking)** — on every price write, pollers mark each active playbook leg `hit` / `missed` / `pending` and record the actual extreme reached — no LLM call, no cron. Gives a live "on track / diverged" read on a playbook *between* Analyzes, and feeds both the post-mortem (item 1) and the signal lab (`../../BUILD_QUEUE.md` → Batch X8) with realized-vs-predicted outcomes. The FE already renders forward-compatible per-leg status glyphs (shipped in 14g). *(Was MVP Batch 14h Half A; deferred behind validating base playbook quality on real tickers before building the evaluation loop.)*
8. **Refine follow-up (directed re-analysis of a live playbook)** — a second, manually-triggered **Refine** analyze mode, distinct from Re-analyze fresh: sends the current feature pack + the prior playbook + its realized leg outcomes (from item 7) + an anti-anchoring instruction → a revised playbook that supersedes the prior one and records `analyses.refined_from_analysis_id` (column already shipped in migration `010_playbook.sql`). TickerDetail shows two buttons when an active signal exists — Refine vs. Re-analyze fresh. *(Was MVP Batch 14h Half B.)*
9. **Signal-generation controls (Settings + Analyze button)** — the app-level knobs that gate the Analyze / signal-generation flow. Pulled out of MVP Batch 15 (Settings) because they're part of the LLM-analysis track, not the app-shell settings (IB connection, profit-zone threshold, theme, account) that Batch 15 actually ships. All persist through the already-built validated path `PUT /api/user/preferences` (Batch 15 wired `risk_flag_config` + `profit_zone_threshold_pct` through it; these reuse it):
   - **Signal-generation quality threshold** — minimum `signalQuality` below which the engine records `no_signal` instead of persisting a weak playbook. Acts at generation time (distinct from any future Alerts-feed display filter). The column `user_preferences.signal_threshold` exists (default 70) but is **not enforced** by `signalEngine` today — wiring the gate (compare *after* the CRITICAL risk-flag cap, then null the signal) is part of this item. Note the live consequence: with current signal quality landing ~50–65, a default-70 floor would suppress most signals until the user lowers it, so revisit the default alongside the quality-sharpening work.
   - **Signal min market value** — `user_preferences.signal_min_market_value`; already enforced by `signalEngine`, but its Settings control isn't built.
   - **Suppressed symbols** — `user_preferences.suppressed_symbols`; the server already skips generation for suppressed tickers, but (a) the Settings list editor and (b) hiding/disabling the **Analyze button on TickerDetail** for suppressed symbols are unbuilt (the latter needs the suppressed list threaded into the TickerDetail data flow). Both belong here.
